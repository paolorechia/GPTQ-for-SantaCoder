...
def make_quant(module, names, bits, groupsize, name=""):
    ...
    for attr in dir(module):
        ...
        if name1 in names:
            delattr(module, attr)
            setattr(
                module, attr, QuantLinear(bits, groupsize, tmp.in_features, tmp.out_features, tmp.bias is not None)
            )
...
class QuantLinear(nn.Module):
    def __init__(self, bits, groupsize, infeatures, outfeatures, bias, kernel_switch_threshold=128, is_cuda=is_cuda):
        ...
        if self.bits in [2, 4, 8]:
            self.register_buffer(
                "wf", torch.tensor(list(range(0, 32, self.bits)), dtype=torch.int32).unsqueeze(0), persistent=False
            )
        elif self.bits == 3:
            self.register_buffer(
                "wf",
