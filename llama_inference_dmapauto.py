...
def load_quant(model, checkpoint, wbits, groupsize, device_map):
    ...
    model = accelerate.load_checkpoint_and_dispatch(
        model, checkpoint, device_map=device_map, no_split_module_classes=["LlamaDecoderLayer"]
    )
    model.seqlen = 2048
    print("Done.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--do_sample", action="store_true", help="Perform multinomial sampling (slow) to produce varied output."
    )
    ...
    parser.add_argument(
        "--keep_alive", action="store_true", help="Keep the process alive so others can share the loaded model."
    )
